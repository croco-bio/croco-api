...
public class FIMOHandler extends TFBSHandler{
	...
	public HashMap<String,List<de.lmu.ifi.bio.crco.intervaltree.peaks.TFBSPeak>> readHits(File tfbsFile) throws IOException{
		...
		while ( (line = br.readLine() )!= null) {
			...
			for(Transcript transcript :gene.getTranscripts()){
				Integer distance = null;  // Peak to TSS start
				if ( gene.getStrand().equals(Strand.Plus)){
					distance = Math.abs(absolutMiddle-transcript.getStart());
				}else{
					distance = Math.abs(absolutMiddle-transcript.getEnd());
				}
			...
			TFBSPeak peak = new TFBSPeak(region.getChrom(),new Entity(mappedFactor,(Transcript)bestTranscript,currentMinDistance,pValue,absolutStart,absolutEnd);
	...
	public static void main(String[] args) throws Exception{
		...
		HashMap<String, List<TFBSPeak>> matchTree = new FIMOHandler(tfbsRegion,pValueCutOf,null).readHits(tfbsFiles);
